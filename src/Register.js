...
export default class Register extends Component {
    ...
    _checkExistUser() {
        return fetch('http://103.252.1.5:8089/SocialPlaceServer/Action/DoCheckUserName.Action', {
            ...
            .then((responseJsons) => {
                ...
                console.log("_checkExistUser - The 2nd .then in func : ", responseJsons);
                if (responseJsons.code == 1) {
                    // console.log("The responseJsond 2nd .then in func _checkLogin : ", responseJsons);
                    this._createNewUser();
    ...
    _createNewUser() {
        return fetch('http://103.252.1.5:8089/SocialPlaceServer/Action/DoRegister.Action', {
            method: 'POST',
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
            ...
            .then((response) => response.json())
            .then((responseJson) => {
                console.log("_createNewUser : " ,responseJson);
        ...
        return (
            <View style={{ justifyContent: "center", alignItems: "center" }}>
                <View style={{ height: 50, width: "100%", backgroundColor: "skyblue", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Text style={{ fontFamily: "Roboto", fontSize: 18, color: "white" }}>ĐĂNG KÝ</Text>
                ...
                <View style={{ width: "100%", justifyContent: "center", alignItems: "center" }}>
                    <TouchableOpacity
                        ...
                        <View style={{ backgroundColor: "#49B68C", height: 30, width: 200, borderRadius: 15, justifyContent: "center", alignItems: "center" }}>
                            <Text style={{ fontFamily: "Roboto", color: "white" }}>TẠO TÀI KHOẢN</Text>
                        </View>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => Alert.alert(this.state.dataUser.UserName, this.state.dataUser.Password,
                            this.state.dataUser.LastName, this.state.dataUser.FirstName, this.state.dataUser.Email)}
                        // onPress={() => this.toRegisterAppScreen()}
                        style={{ height: 30, width: 200, justifyContent: "center", alignItems: "center" }}>
                        <Text style={{ fontFamily: "Roboto", color: "#49B68C" }}>LOG STATE</Text>
                    </TouchableOpacity>
